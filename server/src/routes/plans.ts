...
router.post('/', requireAdmin, async (req: Request, res: Response) => {
  ...
  const plan = await prisma.plan.create({ data: { name, eventLimit: event_limit, priceSek: price_sek, description: description || null } });
...
router.put('/:id', requireAdmin, async (req: Request, res: Response) => {
  ...
  const { name, event_limit, price_sek, description } = req.body;
  const plan = await prisma.plan.update({ where: { id: req.params.id }, data: { name, eventLimit: event_limit, priceSek: price_sek, description } });
